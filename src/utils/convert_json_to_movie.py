...
async def process_movies_json(responses: list, update: Update) -> list:
    movies = []
    for movie in responses:
        link = "https://www.kinopoisk.ru/film/%s/" % movie.get("id")
        directors = [person.get("name") for person in movie["persons"] if person.get("enProfession") == "director"]
        directors = ",".join(directors) if len(directors) > 1 else directors[0]
        description = movie.get("description")
        if description is not None:
            description = description.encode("utf-8").decode("utf-8").replace("\xa0", " ")
        else:
            description = ""
